# Local-dev seed for the menu app. Runs against the docker-compose
# Zitadel on `localhost:8080` (`docker-compose.yml::services.zitadel`),
# authenticated with the menu-sa PAT FirstInstance writes to
# `products/menu/.zitadel-bootstrap/menu-sa.pat`.
#
...
# `.env.local` in dev has the exact shape of prod's runtime env. No
# mind-shift between dev and prod: same keys, same source-of-truth
...
variable "zitadel_pat" {
  description = "Personal access token for the menu-sa machine user. Read from .zitadel-bootstrap/menu-sa.pat by scripts/dev.sh."
  type        = string
  sensitive   = true
...
resource "local_file" "env" {
  ...
  content  = <<-EOT
    # AUTO-GENERATED by products/menu/infra/dev/tofu/main.tf.
