...
3. Copy the token — you'll paste it into `.env.deploy`.
...
`.env.deploy` has 7 inputs you fill in and 4 secrets you generate. Use `openssl rand -hex 32` four times and paste into `STATE_PASSPHRASE` / `BETTER_AUTH_SECRET` / `POSTGRES_PASSWORD` / `MINIO_ROOT_PASSWORD`:
...
Keep `.env.deploy` in your password manager — it holds everything needed to redeploy from scratch. The file is gitignored.
...
All are direct `kamal` calls — the Makefile only loads `.env.deploy`, exports its values, and resolves the gem-bin PATH so subprocesses find `kamal`.

For ad-hoc kamal commands (e.g. `kamal app stop`, `kamal accessory exec`), source `.env.deploy` first:
...
- **`.env.deploy`** → Makefile `-include` + `export` → visible to every `tofu`/`kamal` subprocess.
...
- **App secrets** (BETTER_AUTH_SECRET, POSTGRES_PASSWORD, etc.) → `.kamal/secrets` references `$VAR` form, which Kamal evaluates against the env (sourced from `.env.deploy` via the Makefile).
...
.env.deploy.example                  infra template — copy to .env.deploy (Tofu + Kamal; NOT loaded by Next)
...
**`make deploy` errors with `key not found` early on** — `.env.deploy` is missing or a required key isn't filled. Copy `.env.deploy.example` and fill in every value.
