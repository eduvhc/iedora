...
9. **Resource naming grammar.** `<provider>_<noun>.<role>_<qualifier>`. Examples: `cloudflare_dns_record.menu_apex`, `cloudflare_r2_bucket.assets`, `tailscale_federated_identity.ci`. Predictable grammar = LLM can import addresses correctly without guessing.
