...
func main() {
	...
	step(4, "write products/menu/.env.local from tofu output")
	envContent := captureIn(tofuDir, "tofu", "output", "-raw", "menu_env_file")
	envPath := filepath.Join(repoRoot, "products/menu/.env.local")
	header := "# AUTO-GENERATED by `bun run dev` (via infra/modules/menu_env).\n" +
		"# Do not edit by hand — your changes will be overwritten on the next seed run.\n\n"
	if err := os.WriteFile(envPath, []byte(header+envContent+"\n"), 0o600); err != nil {
		fail("write %s: %v", envPath, err)
	}
