...
//
// All callers run inside `infra/iac/`, the parent of the Tofu root.
// We bind that explicitly via cmd.Dir so the binary works regardless
// of where the operator invoked it from (e.g. from the repo root via
// `task infra:up`, which itself shells through `bin/iedora`).
